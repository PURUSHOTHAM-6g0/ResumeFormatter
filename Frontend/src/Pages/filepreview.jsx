...
const validateResumeData = (data) => {
  console.log("Raw data received for validation:", data) // Debug log
  ...
  // Validate links array with more lenient validation
  let validatedLinks = []
  console.log("Raw links data:", data.links) // Debug log

  if (Array.isArray(data.links)) {
    validatedLinks = data.links.filter((link) => {
      console.log("Validating link:", link) // Debug log
      if (!link || typeof link !== "object") {
        console.log("Link rejected: not an object")
        return false
      }
      const hasType = link.type && typeof link.type === "string" && link.type.trim() !== ""
      const hasUrl = link.url && typeof link.url === "string" && link.url.trim() !== "" && link.url !== "Not available"
      console.log("Link validation result:", { hasType, hasUrl, link })
      return hasType && hasUrl
    })
  } else if (data.links && data.links !== "Not available") {
    // Handle case where links might be a string or other format
    console.log("Links is not an array, attempting to parse:", data.links)
    try {
      if (typeof data.links === "string") {
        const parsed = JSON.parse(data.links)
        if (Array.isArray(parsed)) {
          validatedLinks = parsed.filter((link) => {
            return link && typeof link === "object" && link.type && link.url && link.url !== "Not available"
          })
        }
      }
    } catch (e) {
      console.log("Failed to parse links string:", e)
    }
  }

  console.log("Final validated links:", validatedLinks) // Debug log
  ...
  console.log("Final validated result:", result) // Debug log
  return result
}
...
export default function PreviewPage() {
  ...
  useEffect(() => {
    if (location.state?.jsonData) {
      ...
      try {
        console.log("Raw jsonData from location.state:", jsonData)
        console.log("Raw allData from location.state:", allData)
